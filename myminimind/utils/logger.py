...
def setup_logger():
    ...
    if rank == 0:
        ...
        # 文件输出（无颜色）
        logger.add(LOG_FILE, format="{time:YYYY-MM-DD HH:mm:ss.SSS} | {level} | {file.path}:{line} | {function} | {message}", level="DEBUG", encoding="utf-8", enqueue=True, backtrace=True, diagnose=True)
